...
is a clear jump. Most of the gain comes from two things: the surface
specific ELO, and the closing-odds probability used as a feature. The
closing-odds feature alone gives around \(+2\) points and shows up as
the fourth most important feature, just after the ELO difference, the
log-ranking difference, and the surface ELO difference.
...
Looking at the feature importance of the trained model, the strongest
features are the ELO difference, the log-ranking difference, the
surface ELO difference and the BookMaker-implied probability. Recent
form, head-to-head and serve statistics also help, but with much
smaller weight. In tennis, most of the signal is captured by a small
number of strong, well-known features, and adding many small features
only helps a little.
